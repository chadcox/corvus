...
**CSV exports** (`timeline/export`, `evidence/{id}/hashes/export`): both stream rows through `app/util/csv_export.py`, which prefixes cells that a spreadsheet would parse as a formula (leading `=`, `+`, `-`, `@`, their full-width forms, or a whitespace control) with `'`. While enabled, all fields are quoted so embedded locale delimiters remain inside the logical cell. Plain numbers are exempt so negative values stay readable. Escaping applies at the CSV boundary only — stored rows, JSON responses, and worker COPY serialization keep the original bytes. Disable with `CSV_EXPORT_FORMULA_ESCAPE=false`.
