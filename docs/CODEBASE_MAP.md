---
last_mapped: 2026-08-12T23:34:16Z
total_files: 268
total_tokens: 340934
---
...
> Auto-generated by Cartographer. Last mapped: 2026-08-12
...
apps/
  ...
  web/                  React 19 + Vite SPA
    src/
      ...
      components/       Investigation views, GlobalSearch, ConfirmDialog, status/detection panels
      hooks/            Shared roving-row keyboard navigation and dialog focus management
...
docs/                   Operations/product docs, architecture map, screenshots, design-remediation work orders
...
- **Chainsaw** (`worker/chainsaw/`): priority-ranked EVTX selection (Security/Sysmon/PowerShell... up to the effective `CHAINSAW_EVTX_MAX=64`, with values below 1 clamped to 1), batched parallel hunts, hits correlated back to timeline events by `(EventID, RecordID, Computer)`. Selection counts and batch success counts feed a count-only `Partial detection coverage:` final-job note when the ceiling omits EVTX or a batch fails/times out, including when there are zero hits. This is detection coverage only: EVTX outside the Chainsaw selection may still be timeline-parsed, and the package-retention predicate remains limited to `Partial parse:` notes. `dfir` Sigma profile builds a hardlinked filtered rules cache. Mutually exclusive with in-process Sigma via `CHAINSAW_ENABLED && CHAINSAW_INCLUDE_SIGMA`.
...
**Purpose**: Analyst SPA. React 19 + react-router 7 + `@tanstack/react-virtual`. No state library — local state + prop drilling; no CSS framework — hand-written `App.css` (2806 lines) plus global tokens/elements in `index.css`.
...
├── sidebar: upload, source selection/details, hash/YARA, stat pivots, ingest history
├── main: IngestStatusPanel, case summary, SigmaFindingsPanel, GlobalSearch
├── tabs: TimelineView | ObjectView | DiskView | MftView* | BrowserView*
└── dialogs: ConfirmDialog plus portalled evidence/history dialogs using useDialog
    (* shown only when stats.mft_count / browser_count > 0)
```

**View data flow**: `CaseDetailPage` owns source/tab/pivot state; each view owns its filters, selection, loading, and direct calls through `api/client.ts`. Findings/search pivot into Timeline, Objects, or Disk; Timeline entities and Object related events pivot back across views.

**TimelineView**: virtualized sparse page cache (`PAGE_SIZE=10000`), scrollbar-thumb bottom-jump special case, and sqrt-scaled SVG histogram (`TimelineChart`). Unloaded virtual indexes render placeholders, so row focus and selection cannot assume a flat array. **ObjectView**: entity list plus related-event list. **MftView**: server pages of 500, server-side path search across the whole source (`q` + `mft_only`, 300ms debounce, page reset, count from `timeline/count` with the same filters), client-side sort within the loaded page only, timestomping flag when `$STANDARD_INFO < $FILE_NAME`. **DiskView**: tree/search plus 512-byte hex preview and hashes. **BrowserView**: category filters and load-more paging (200).

**CSS/design**: `App.css` owns component/layout rules; `index.css` owns tokens and global element rules. `main.tsx` imports `App.css` first and `index.css` second, so equal-specificity global rules win. `PRODUCT.md` governs evidence language; `DESIGN.md` governs palette, typography, and interaction rules. Syne, Libre Franklin, and Red Hat Mono are installed through locked `@fontsource` packages and imported at the eight weights used by the UI; runtime font requests stay local.

**Keyboard collections/dialogs**: `useRowNavigation` provides roving `tabIndex`, arrow/Home/End/Page navigation, and Enter/Space activation across Timeline, both Object lists, Disk, MFT, and Browser. `useDialog` portals content dialogs outside `#root`, sets the app inert, locks body scroll, traps focus dynamically, handles Escape, and restores the opener. `ConfirmDialog` retains its specialized two-button trap. Detection details and container logs remain separate unmanaged dialogs.
...
**E2E** (`apps/web/e2e/`): `helpers.ts` mocks nearly the whole API surface; `npm run test:e2e:mocked` runs smoke, analyst, and control-panel suites. Coverage includes 25k-row paging, all six keyboard row collections, both CaseDetail content dialogs, evidence-safe error/partial states, focus/contrast states, and offline font loading. `backend-analyst-flow.spec.ts` is opt-in with `PLAYWRIGHT_BACKEND_E2E=1`; `screenshots.spec.ts` regenerates README images.
...
- **Web CSS cascade**: `App.css` loads before `index.css`; narrow global element rules instead of using `!important` or specificity escalation.
- **Web evidence language**: absence of detections or failed retrieval is not a clean/empty conclusion; follow `PRODUCT.md` vocabulary and keep error/loading/empty states mutually exclusive.
- **Web keyboard rows**: list collections use named listbox/option semantics; native table rows keep table semantics and expose `aria-current`. All share `useRowNavigation`.
- **Web content dialogs**: portal outside `#root` before using `useDialog`; otherwise the inert app root disables the dialog itself.
...
- **Web modal split**: CaseDetail evidence/history dialogs and `ConfirmDialog` are managed; detection details and container logs remain separate unmanaged `aria-modal` implementations.
- **Web CSS collision**: global button hover rules in later-loaded `index.css` can override composite button states in `App.css`; `:where()` exclusions preserve variant specificity.
- **Web Playwright image drift**: `docker-compose.yml` pins Playwright `v1.60.0`, while the host-installed runner currently requires `v1.61.0`; use a matching image or update both together before relying on the compose test service.
