...
**Timeline export truncation** (`timeline/export`): capped at the fixed module constant `EXPORT_MAX` (`50000`) in `app/routers/timeline.py` — deliberately not a setting. The route runs exactly one query, `.limit(EXPORT_MAX + 1).all()`, and materializes it *before* constructing the `StreamingResponse`; `truncated`, the row-count header, and the CSV rows all read from that one list. This is the ordering constraint that matters: headers freeze when the response object is built while the generator runs later, so any second query (a `COUNT`, a probe) could disagree with the bytes that actually shipped and let a header claim "complete" over a partial file. Buffering is bounded by construction — at most 50,001 five-column scalar tuples, no ORM identity map, same order of magnitude as the streamed body. The `+ 1` row is a truncation sentinel and is never written, so the header count always equals the body length. Rows are the filter's timestamp-ascending order, so a capped export keeps the *oldest* matches. Disclosure is header-only (`X-Corvus-Export-Truncated`, `-Row-Limit`, `-Row-Count`); the CSV body stays byte-identical to an untruncated export of the same rows and carries no warning row. Caveat: `Access-Control-Expose-Headers` is set **per-route** rather than on the global `CORSMiddleware` in `main.py`, which is currently constructed without `expose_headers`; configuring `expose_headers` there would overwrite the route's value and silently blind cross-origin callers, so the two must be changed together. Because the timeline router requires a bearer token, the web export runs through `api.downloadTimelineCsv` (authenticated fetch → blob → synthetic anchor) instead of linking to the URL; an anchor navigation would be rejected with 401 and could not read the metadata headers. There is no pre-download prompt — the browser cannot know the outcome in advance — so `TimelineView` reports it afterwards in an always-mounted `role="status"` region as complete, partial (exact rows written, oldest by timestamp, no claim about the unreported total), unverified when the headers are absent, or the failure reason. `timeline/count` returns only `count`.
